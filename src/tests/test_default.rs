...
fn test_default_fixed_rate() {
    ...
    let pool = create_blend_pool(&e, &blend_fixture, &bombadil, &usdc_client, &xlm_client);
    let pool_client = PoolClient::new(&e, &pool);
    let fee_vault = register_fee_vault(&e, &bombadil, &pool, &usdc, 2, 40_0000, None);
    let fee_vault_usdc_client = FeeVaultClient::new(&e, &fee_vault);
    ...
    // pass 1m day to accrue some fees (approx 0.329% gain w/ 4% fixed rate on vault, pool suppliers earning 5%)
    e.jump_time(30 * 86400);
    ...
    // check fee accrual amount is not dust
    let pre_default_vault = fee_vault_usdc_client.get_vault();
    assert!(pre_default_vault.admin_balance > 0);
    ...
    // estimate expected loss frodo and samwise should take, as a percentage, given the 1m gain
    // and 10% default
    let expected_loss = 1_0032876i128
        .fixed_mul_floor(0_9000000, SCALAR_7)
        .unwrap_optimized();
    ...
    // validate admin `b_token` balance was not impacted by the default
    let post_default_vault = fee_vault_usdc_client.get_vault();
    assert_eq!(
        pre_default_vault.admin_balance,
        post_default_vault.admin_balance
    );
    assert!(pre_default_vault.b_rate > post_default_vault.b_rate);
